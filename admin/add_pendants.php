...
include("../connection.php");
    ...
    if(isset($_GET['del'])){
	$delid = $_GET['del'];
	$del1 = "DELETE FROM pendants WHERE PRO_ID='$delid'";
	$del2 = mysqli_query($conn,$del1);
	echo"<script>window.open('add_pendants.php','_self')</script>";
}
?>
<html>
<head>
      <title>Adding New Product-Pendants</title>
...
<form action="add_pendants.php" method="POST" enctype="multipart/form-data">
    <table class="insert_table"align="center" cellpadding="10" width="750" border="2" bgcolor="#093145" style="color:lightgrey;">
	    <tr align="center">
            <td colspan="8"><h2>Add New Product-Pendants</h2> </td>
        </tr>		
		<tr>
...
if(isset($_POST['insert_product'])){
	...
	if($metal == "g"){
	move_uploaded_file($product_image_tmp,"../img/pendants/$product_image");      /// image
	}else{
	move_uploaded_file($product_image_tmp,"../img/pendants/$product_image");      	
	}
	
	$insert_product =  "INSERT INTO pendants(IMAGE,TITLE,PRICE,METAL)
...
?>		
		<tr>
			...
			if($pro_metal == "g"){
			?>
				<td align="center"><img src="../img/pendants/<?php echo $pro_metal; ?>/<?php echo $pro_img; ?>" alt="<?php echo $pro_img; ?>" width="200" height="200" style="border-radius:15px; background-color:white;"></td>
			<?php
			}else{
			?>
				<td align="center"><img src="../img/pendants/<?php echo $pro_metal; ?>/<?php echo $pro_img; ?>" alt="<?php echo $pro_img; ?>" width="200" height="200" style="border-radius:15px; background-color:white;"></td>
            ...
            <td><button type="button" onclick="window.location='add_pendants.php?del=<?php echo $pro_id; ?>';" >DELETE</button></td>
	</tr>
<?php
}	
?>
